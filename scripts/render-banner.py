...
Runs `lindisfarner sample.txt --pilcrows --drolleries -c always` and converts
its ANSI output into a self-contained SVG that mirrors a real terminal
rendering: the gold illuminated initial, the ornate ❦ frame, a marginal
drollery, red ¶ pilcrows at paragraph breaks, and the wrapped body text.
...
RED = "#e5675a"  # rubric / pilcrow  (ANSI 31)
...
def ansi() -> str:
    ...
    out = subprocess.run(
        cmd + ["sample.txt", "-c", "always", "--pilcrows", "--drolleries"],
        cwd=ROOT,
        capture_output=True,
        text=True,
        check=True,
...
def style(codes, bold):
    ...
    if 31 in codes:
        return (RED, bold)  # rubricated words and pilcrows
    return (INK, bold)
...
def main():
    ...
    s = [
        "<!-- Generated by scripts/render-banner.py from lindisfarner sample.txt with pilcrows and drolleries enabled. Do not edit by hand. -->",
